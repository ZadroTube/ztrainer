...
export function CoachChat() {
  const { coachMessages, sendCoachMessage } = useWorkoutData();
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-290px)] min-h-[400px] bg-slate-950/40 border border-slate-800/80 rounded-2xl overflow-hidden shadow-2xl relative">
                ...
                >
                  <div className={`flex items-start gap-2 max-w-[85%] ${isUser ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className="flex flex-col">
                      ...
                      {/* Timestamp */}
                      <span className={`text-[8px] text-slate-500 mt-1 ${isUser ? 'text-right mr-1' : 'ml-1'}`}>
      ...
      <form onSubmit={handleFormSubmit} className="border-t border-slate-850/80 bg-slate-900/40 p-3 flex gap-2">
        <input
          ...
          disabled={loading}
          className="flex-1 bg-slate-950 border border-slate-850 hover:border-slate-800 focus:border-emerald-500/50 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none transition-all disabled:opacity-50"
        />
        <button
          type="submit"
